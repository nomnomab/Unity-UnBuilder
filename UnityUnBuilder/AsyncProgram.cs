...
public static class AsyncProgram {
    ...
    static async Task RunConversion(ToolSettings settings) {
        ...
        var versionGreaterThan2018 = gameData.ProjectVersion.GreaterThanOrEquals(2018);
        Console.WriteLine($"{gameData.ProjectVersion} >= 2018? {versionGreaterThan2018}");
        ...
        if (versionGreaterThan2018) {
            ...
            if (packageTree == null || packageTree.Nodes.Count == 0) {
                var packages = new UnityPackages() {
                    Packages = []
                };
                
                // < 2018 is dumb and doesn't have a package manager
                if (versionGreaterThan2018) {
                    packages = await packageDetection.GetPackagesFromVersion(settings, newProjectPath);
                }
                ...
                await Recompile(projectPath, unityPath);
            }
        ...
        var builtinDb = versionGreaterThan2018 ? await ExtractGuids(
            settings,
        ...
        var builtInTypes = versionGreaterThan2018 ? await ExtractTypes(
            unityPath.GetBuiltInPackagesPath(),
    ...
    private static async Task Recompile(string projectPath, UnityPath unityPath) {
