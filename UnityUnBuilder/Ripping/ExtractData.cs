...
public record ExtractData {
    ...
    public async Task CopyAssetsToProject(ToolSettings settings, string secondProject, HashSet<string> fileBlacklist, HashSet<string> folderBlacklist, GuidDatabase guidDb, bool testFoldersOnly) {
        ...
        //! testing
        // folderBlacklist.Add(Path.GetFullPath(Path.Combine(firstPath, "Mesh")));
        // folderBlacklist.Add(Path.GetFullPath(Path.Combine(firstPath, "AudioClip")));
        // folderBlacklist.Add(Path.GetFullPath(Path.Combine(firstPath, "Cubemap")));
        // folderBlacklist.Add(Path.GetFullPath(Path.Combine(firstPath, "VideoClip")));
        ...
        foreach (var name in PackageDatabase.ExcludeAssembliesFromProject) {
            ...
            foreach (var dir in Directory.GetDirectories(scriptsFolder, name, SearchOption.TopDirectoryOnly)) {
                ...
                AnsiConsole.WriteLine($" - [CopyAssetsToProject] excluding dll match: \"{dir}\"");
            }
        }
        ...
        foreach (var name in PackageDatabase.IgnoreAssemblyPrefixes) {
            ...
            foreach (var dir in Directory.GetDirectories(scriptsFolder, $"{name}*", SearchOption.TopDirectoryOnly)) {
                folderBlacklist.Add(dir);
                fileBlacklist.Add(dir + ".meta");
                AnsiConsole.WriteLine($" - [CopyAssetsToProject] excluding starting prefix: \"{dir}\"");
            }
        }
        ...
        foreach (var folder in folderBlacklist) {
            ...
            foreach (var file in files) {
                ...
                AnsiConsole.WriteLine($" - [CopyAssetsToProject] excluding in blacklist: \"{realFile}\"");
            }
        }
        ...
        if (testFoldersOnly) {
            AnsiConsole.MarkupLine("[red]Using test folders only[/]");
            
            //! testing
            // foreach (var file in Directory.GetFiles(firstPath, "*.*", SearchOption.AllDirectories)) {
            //     var extension = Path.GetExtension(file);
            //     if (extension is ".mp3" or ".ogg" or ".wav" or ".mp4" or ".mov" or ".mpeg" or ".avi") {
            //         fileBlacklist.Add(Path.GetFullPath(file));
            //     }
            // }
