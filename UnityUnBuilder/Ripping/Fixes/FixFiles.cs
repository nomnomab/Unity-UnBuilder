...
public static class FixFiles {
    ...
    public static async Task CopyOverCustomFiles(ToolSettings settings) {
        ...
        foreach (var (from, to) in gameSettings.Files.CopyPaths ?? []) {
            ...
            AnsiConsole.WriteLine($"Copying {fromPath, 4} to {Utility.ClampPathFolders(toPath)} from gameSettings.Files.CopyPaths");
    ...
    public static void ParseTextFiles(ExtractData extractData) {
        ...
        foreach (var file in txtFiles) {
            Console.WriteLine($"Decoding {Utility.ClampPathFolders(file)}");
    ...
    public static void FixShaders(ExtractData extractData) {
        ...
        foreach (var file in shaderFiles) {
            Console.WriteLine($"Fixing {Utility.ClampPathFolders(file)}");
    ...
    public static void FixDuplicateAssets(ToolSettings settings) {
        ...
        foreach (var path in toProcess) {
            ...
            foreach (var file in files) {
                ...
                AnsiConsole.WriteLine($" - fixed {Utility.ClampPathFolders(file)}");
            }
        }
    }
    ...
    public static void FixAssetNames(ToolSettings settings) {
        ...
        foreach (var path in toProcess) {
            ...
            foreach (var file in files) {
                AnsiConsole.WriteLine($" - checking {Utility.ClampPathFolders(file)}");
