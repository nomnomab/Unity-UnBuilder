...
3. Opens Unity and fetches the packages".TrimStart()) {
    ...
    public void ApplyGameSettingsPackages(ToolSettings settings, PackageTree packageTree) {
        ...
        Console.WriteLine($"{packages.Overrides.Count} packages to override");
        
        foreach (var (id, version, exclude) in packages.Overrides) {
            Console.WriteLine($"[override] {id} @ {version}, exclude? {exclude}");
