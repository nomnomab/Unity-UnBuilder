...
public static class Extract {
    ...
    public static async Task<ExtractData> ExtractAssets(ToolSettings settings) {
        ...
        if (shadersRoot != null) {
            ...
            foreach (var shader in shaders) {
                ...
                AnsiConsole.WriteLine($"Copied:\nfrom:{Utility.ClampPathFolders(shader)}\nto: {Utility.ClampPathFolders(newPath)}");
            }
    ...
    private static async Task<string> ExtractDecompiledShaders(ToolSettings settings) {
        var (config, gameData, _) = ExtractGameData(settings, null, true);
        ...
        if(!settings.ProgramArgs.SkipAssetRipper) {
            ...
            await AnsiConsole.Status()
                ...
                async ctx => await WaitForAssetRipper(ctx, "Decompiling shaders...", shadersOutputRoot, new ExportHandler(decompiledShadersConfig), gameData)
            );
        ...
        foreach (var shader in shaders) {
            ...
            if (!File.Exists(metaFile)) {
                AnsiConsole.WriteLine($"{shader} could not decompile");
                continue;
            }
            ...
            text     = text.Replace("GpuProgramID", "// GpuProgramID");
